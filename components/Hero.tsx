...
export default function Hero() {
  return (
    <section id="top" className="relative overflow-hidden bg-ink pt-32 text-paper md:pt-36">
      ...
      <div className="relative mx-auto grid max-w-[1240px] grid-cols-1 items-center gap-12 px-6 pb-16 md:px-8 md:pb-20 lg:grid-cols-[1.08fr_0.92fr] lg:gap-16 lg:pb-24">
        ...
        <Reveal className="relative order-first lg:order-last" delay={150}>
          <div className="relative mx-auto max-w-[480px] rounded-[2rem] border border-paper/15 bg-paper/[0.06] p-6 shadow-2xl backdrop-blur-sm md:p-8">
            <div className="absolute -right-3 -top-3 rounded-full bg-gold px-4 py-2 font-mono text-[0.65rem] uppercase tracking-widest text-ink">Heritage • Community • Development</div>
      ...
      <div className="relative border-y border-paper/10 bg-black/10 py-4 overflow-hidden"><div className="marquee-track font-mono text-[0.78rem] uppercase tracking-wide text-paper/65">{[...PILLARS, ...PILLARS].map((p, i) => <span key={i} className="flex items-center gap-3">{p}<span className="text-gold text-[0.6rem]">◆</span></span>)}</div></div>
    </section>
  );
}
